...
get.infobox.coin <- function(x){
  ...
  df1 <- get.data() # run the scraping function above and assign that data.frame to a variable
  df1 <- df1$`Name`[1]  # assign the first value of the % gain column to same variable
  df1   # return value
...
ui <- dashboardPage(
  ...
  dashboardHeader(title = "CoinMarket Gainers"),
  
  # S I D E B A R
  
  dashboardSidebar(
    
    h5("An interactive dashboard that pulls the top gainers from the last hour from
